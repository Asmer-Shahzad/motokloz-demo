...
<section class="fleet-section">
    <div class="container">
        ...
        <div class="row g-4">
            <div class="col-lg-3 col-md-4 search-fill" data-aos="fade-right" data-aos-duration="700">
        ...
        request('selected_fuel_type') == 'PREMIUM RECOMMENDED' ? 'selected' : '' }}>
            ...
            <div class="col-lg-9 col-md-8 main-right">
                    ...
                    <div class="col-lg-4 col-sm-6 vehicle-card" data-id="{{ $recent_vehicle->id }}"
                        ...
                        <div class="modern-car-card shadow-sm">
                            <div class="car-card-top">
                                ...
                                $slug = Str::slug($vehicleName, '-') ?: 'vehicle';
                                $detailUrl = route('inventory_product_details', [
                                'name' => $slug,
                                'id' => $recent_vehicle->id
                                ]);
                                @endphp
                                <a href="{{ $detailUrl }}">
                                ...
                                <div class="badge-mileage d-flex align-items-center">
                                    @php
                                        ...
                                        if(empty($cardTitle)) {
                                            $cardTitle = $recent_vehicle->title ?? '';
                                        }
                                        if(!empty($recent_vehicle->trim)) {
                                            $cardTitle .= ' ' . $recent_vehicle->trim;
                                        }
                                    @endphp
                                    {{ $cardTitle ?: 'Vehicle' }}
                                </div>
                            ...
                            <div class="car-card-bottom">
                                ...
                                {{-- Title: only visible in list view (hidden in grid via CSS) --}}
                                <h5 class="card-list-title">
                                    {{ $recent_vehicle->year }} {{ $recent_vehicle->mfg_auto }}
                                    {{ $recent_vehicle->model }} {{ $recent_vehicle->trim }}
                                </h5>

                                {{-- Location + Mileage same line (list view only, figma style) --}}
                                ...
                                {{-- Mileage row (grid view only — right aligned) --}}
                                @if(!empty($recent_vehicle->mileage) && (float)trim(str_ireplace('km','',$recent_vehicle->mileage)) > 0)
                                ...
                                {{-- Row 2: Circle icons --}}
                                @php
                                // Figma-exact order: 21 icons, 9 per row in list view
                                $allServiceIcons = [
                                // Row 1 (orange)
                                ...
                                // Chat params
                                $chatDealerId = (int) ($recent_vehicle->client_id ?? data_get($recent_vehicle, 'dealer.id') ?? 0);
                                ...
                                {{-- Row 3: Distance (left) + icons (middle) + price/call (right) --}}
                                <div class="card-bottom-row">
                                    ...
                                    @if($displayPrice > 0)
                                    {{-- Chat + phone icons middle, price right --}}
...
{{-- Hidden chat form for POST submission --}}
            ...
            return fetch(`{{ env('diskloz_base_url') }}/api/search_inventory?selected_asset=${encodeURIComponent(type)}&per_page=1`)
                ...
                .then(data => {
                    ...
                    switch (type) {
                        ...
                        case 'HEAVY TRUCK/EQUIPMENT':
                            return data.filters.MfgHeavyTruckEquipment || [];
                        case 'HEAVY DUTY TRAILERS':
                            return data.filters.MfgHeavyDutyTrailer || [];
                        case 'FARM EQUIPMENT':
                            return data.filters.MfgFarmEquipment || [];
                        default:
                            return [];
                    }
                })
        ...
        function applyView(view) {
            ...
            if (view === 'list') {
                ...
                cards.forEach(function(c) {
                    c.classList.remove('col-lg-4', 'col-sm-6');
...
<script>
    ...
    $(document).ready(function() {
        ...
        fetch(DISKLOZ_BASE + '/api/favorites?client_id=' + authId)
            ...
            .then(function(data) {
                var likedIds = new Set((data || []).map(function(item) {
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        var form = document.querySelector('form.search-wrapper') || document.getElementById('vehicleFilterForm');
